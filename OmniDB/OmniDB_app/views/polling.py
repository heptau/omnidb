	...
	try:
		...
		try:
			...
			if v_cmd_type=='export_csv' or v_cmd_type=='export_xlsx':
				...
				v_csv_encoding = getattr(v_session, 'v_csv_encoding', 'utf-8')
				if not v_csv_encoding:
					v_csv_encoding = 'utf-8'
				v_csv_delimiter = getattr(v_session, 'v_csv_delimiter', ';')
				if not v_csv_delimiter:
					v_csv_delimiter = ';'
				f = Spartacus.Utils.DataFileWriter(os.path.join(v_export_dir, v_file_name), v_data1.Columns, v_csv_encoding, v_csv_delimiter)
				f.Open()
